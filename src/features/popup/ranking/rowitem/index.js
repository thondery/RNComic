...
class RowItem extends Component {
  ...
  render () {
    let { data, onPushByBook, onPushByChapter, rowId } = this.props
    ...
    return (
      <TouchableOpacity style={styles.container}
        ...
        <View style={styles.mainView}>
          ...
          <Text style={styles.mainTextStyle}>{_.join(['玄幻', '搞笑', '恋爱'], ' ')}</Text>
          <Text style={styles.mainTextStyle} numberOfLines={1}>{itemData.content}</Text>
        </View>
        <View style={styles.toolView}>
          <Text style={styles.ranking}>{this.formatRanking(rowId)}</Text>
        </View>
      </TouchableOpacity>
    )
  }
